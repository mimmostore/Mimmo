...
const load=async()=>{
  try{
    const [a,b]=await Promise.all([
      fetch('/api/catalog-categories.json',{cache:'force-cache'}),
      fetch('/api/catalog-search.json',{cache:'force-cache'})
    ]);
...
function openCats(){
 ...
 const render=()=>{if(!cats?.length){grid.innerHTML='<div class="local-search-empty">Nessuna categoria disponibile</div>';return;}grid.innerHTML=cats.map(c=>`<a class="local-cat-item" href="/${esc(String(c.href).replace(/^\/+/, '').replace(/\.html$/,'') + '.html')}"><img loading="lazy" decoding="async" src="${esc(c.image)}" alt="${esc(c.name)}"><span>${esc(c.name)}</span></a>`).join('')};
...
function openSearch(input,box){
 ...
 box.querySelectorAll('[data-search-index]').forEach((el,i)=>el.onclick=()=>{
   const x=res[i];
   if(x && window.MimmoGallery && Array.isArray(x.gallery) && x.gallery.length){window.MimmoGallery.open(x.gallery,x.title);}
 });
}
function setup(){
